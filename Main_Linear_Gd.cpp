...
//////////////////////////Linear Gd,x_tr_std and x_test_std need to _T()
    ...
    auto X_tr_std=fit_std.tranform(X_tr)._T();

    auto leg=LinearRegression<double>();
    cout << X_tr_std.shape() << endl;
    cout << y_tr._size() << endl;
    auto fit_gd_=leg.fit_gd(X_tr_std, y_tr, 0.08);

    auto x_test_std=fit_std.tranform(X_test)._T();
    auto gd_score=fit_gd_.score(x_test_std, y_test);
